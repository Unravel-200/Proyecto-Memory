...
La matriz contiene 40 PASS, 2 BLOCKED y 42 IDs `NOT RUN`.
...
- No se ejecutaron todavía look, velocidad recta, sprint, crouch, salto, cambio
  ...
  rendimiento. La matriz tiene 40 PASS, 2 BLOCKED y 42 IDs que continúan `NOT RUN`.
...
Sesión `PLR-MOV-005` del 2026-07-28: W + Mayús avanzaron 512.98 cm en un intervalo manual cercano a 0.9 s, coherente con `sprintSpeed=550`; al soltar, `bIsSprinting=false`, `maxWalkSpeed=300` y velocity 0. Evidencia `EV-MOV-SPRINT-01`, log `SPRINT-20260728` (295964 bytes, SHA-256 `6A59B6D4411482E6106F8866F01EDF68E6ED27AC6F222403F8B5A3910AD0AD9C`). Matriz: 30 PASS / 54 NOT RUN. Siguiente: `PLR-CAM-001, cambio a tercera persona.
Sesión `PLR-MOV-006` del 2026-07-28: con la cámara en yaw ~92.6°, W movió el Character principalmente en Y (+247.58 cm) y no en X, confirmando movimiento relativo a la vista; velocity quedó 0 al soltar. Evidencia `EV-MOV-RELATIVE-01`, log `RELATIVE-20260728` (295742 bytes, SHA-256 `59E114F8CF75D6C2C0846CDF63D3A31CA5DBCE6F0E61C089E92649F14A6C5E0D`). Matriz: 31 PASS / 53 NOT RUN. Siguiente: `PLR-CAM-001, cambio a tercera persona.
Sesión de crouch `PLR-CRO-001..003` del 2026-07-28: toque corto, segundo toque y hold de ~0.5 s comprobados. El toque corto dejó `bIsCrouched=true` y cápsula half-height 42 cm frente a 96 cm de pie; segundo toque y liberación del hold devolvieron postura de pie sin sprint residual. Evidencia `EV-CROUCH-01`, log `CROUCH-20260728` (298810 bytes, SHA-256 `AA48623B1195AED97C282AF3CC4D1CD5A76417E32DE4E136C2AE35D1C65D4BBA`). Matriz: 34 PASS / 50 NOT RUN. Siguiente: `PLR-CAM-001, cambio a tercera persona.
Sesión `PLR-CRO-004` del 2026-07-28: desde agachado, mantener C ≥0.5 s y soltar devolvió la postura de pie, sin sprint residual. Evidencia `EV-CROUCH-HOLD-01`, log `CROUCH-HOLD-20260728` (294779 bytes, SHA-256 `24E31A034E9FAFA169E3AF09A4593A06FD8F5A9100DC7CE3EAF4572366CE1B72`). Matriz: 35 PASS / 49 NOT RUN. Siguiente: `PLR-CAM-001, cambio a tercera persona.
`PLR-CRO-005` se intentó el 2026-07-28 retirando el foco mientras C estaba activa. Aunque el personaje terminó de pie, no apareció ningún evento `Canceled` en el log; queda `BLOCKED` mediante `EV-CROUCH-CANCELED-01` hasta instrumentación reproducible. Matriz: 35 PASS / 1 BLOCKED / 48 NOT RUN.
Sesión `PLR-CRO-007` del 2026-07-28: sprint breve seguido de crouch canceló el sprint; runtime confirmó crouch activo, sprint falso, crouchSpeed 180, modo walking y velocity 0 en la repetición válida. Evidencia `EV-CROUCH-SPRINT-01`, log `CROUCH-SPRINT-20260728` (312107 bytes, SHA-256 `067BB888F9EF0D57DBE82515F1F98BD0AC277A9D82DD7C49053835897BA111BC`). Matriz: 36 PASS / 1 BLOCKED / 47 NOT RUN. Siguiente: `PLR-CRO-008`, túnel.
Corrección de look vertical del 2026-07-29: `PMPlayerController::HandleLook` invertía el signo de Y para la convención física del mouse. Se corrigió para que mouse arriba mire arriba con `bInvertLookY=false`; la compilación fue exitosa y el propietario lo confirmó visualmente en PIE (pitch MCP `-13.65°`). Commit `a75aa00`. La iluminación del mapa está en `baad697`.
Sesión `PLR-CRO-008` del 2026-07-29: con la iluminación ya visible, el propietario entró al túnel agachado; UnCrouch bajo techo conservó la postura, y al salir pudo levantarse. Evidencia `EV-CROUCH-TUNNEL-01`, log `TUNNEL-20260729` (295157 bytes, SHA-256 `57ADB9C2731F49AD6570E8DFA4DC1E9693A2EA2C9744CC8FF17A1730A2227DF0`). Matriz: 37 PASS / 1 BLOCKED / 46 NOT RUN. Siguiente: `PLR-CRO-009`, sprint desde agachado.
Sesión `PLR-CRO-009` del 2026-07-29: estando agachado, W + Mayús no activó sprint; `bIsCrouched=true`, `bIsSprinting=false`, crouchSpeed 180. Evidencia `EV-CROUCH-NOSPRINT-01`, log `CROUCH-NOSPRINT-20260729` (294488 bytes, SHA-256 `0DA3F07143E1F84169B7FAB7D6D1785E9196D7ABAECB625AC2C2E358AD1A48C9`). Matriz: 40 PASS / 2 BLOCKED / 42 NOT RUN. Siguiente: `PLR-JMP-001`, salto.
`PLR-JMP-001` se probó parcialmente el 2026-07-29: Espacio saltó y volvió a MOVE_Walking sin estado latente; A/X quedó sin probar por falta de mando físico, así que la fila es `BLOCKED`. Evidencia `EV-JUMP-KEYBOARD-01`, log `JUMP-20260729` (294588 bytes, SHA-256 `B50A832D7E3C980F3C582CF3E9553F2218FF712BAC0942470DB3BEFF242BA5D2`). Matriz: 40 PASS / 2 BLOCKED / 42 NOT RUN. Siguiente: `PLR-JMP-002`, salto desde crouch.
Sesión `PLR-JMP-002` del 2026-07-29: con pausa después de crouch, Espacio levantó y saltó en una sola intención; al caer quedó de pie y sin velocidad residual. Evidencia `EV-JUMP-CROUCH-01`, log `JUMP-CROUCH-20260729` (303415 bytes, SHA-256 `72B198C7092C02AE29D585AF58EDE18BB01F5EAAEF5115B893B1BDC41C64D842`). Matriz: 40 PASS / 2 BLOCKED / 42 NOT RUN. Siguiente: `PLR-JMP-003`, salto bajo túnel.
Sesión `PLR-JMP-003` del 2026-07-29: desde Player Start normal, el propietario llevó el personaje al túnel; Espacio bajo techo no produjo salto pendiente y al salir pudo levantarse. Evidencia `EV-JUMP-TUNNEL-01`, log `JUMP-TUNNEL-20260729` (303019 bytes, SHA-256 `582FE989C1C366FDF1FB22DB4F421910206935B3996DA05B85AD9AA364397267`). Matriz: 40 PASS / 2 BLOCKED / 42 NOT RUN. Siguiente: `PLR-CAM-001`, cambio a tercera persona.
### Sesión más reciente: PLR-CAM-001 (2026-07-29)
...
### Paquete Win64 actualizado — 2026-08-03

El primer intento de `BuildCookRun` reveló que `SetActorLabel` era exclusivo
del Editor. Se protegieron esas etiquetas con `WITH_EDITOR` en
`PMPlayerController.cpp` (`bfb718b`). Después se compiló y cocinó el objetivo
