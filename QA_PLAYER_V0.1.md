...
agachado y cámaras descritos en `EDITOR_SETUP_V0.1.md`. Debe ejecutarse desde la
sección 1 de esa guía, únicamente en una PC adecuada para Unreal Engine 5.8.
...
movimiento, sprint, crouch híbrido, cámaras 1P/3P, colisión, geometría de prueba,
teclado, ratón, mando físico, rendimiento y persistencia de referencias.
...
| SET-02 | `IA_Move` e `IA_Look` son Axis2D; `IA_Sprint`, `IA_Crouch` e `IA_ToggleCamera` son Digital/Bool. Move, Look, Sprint y Toggle no tienen triggers de asset; Crouch usa ninguno por defecto o la excepción Down documentada en `SET-03`. | NOT RUN | |
...
| MAP-02 | `IA_Move` | Gamepad Left Thumbstick 2D-Axis. No añadir otra Dead Zone antes de medir el dispositivo real. | NOT RUN | |
...
| MAP-06 | `IA_ToggleCamera` | V y Gamepad Right Thumbstick Button; sin triggers adicionales. | NOT RUN | |

### Blueprints y mapa

| ID | Comprobación y criterio de aceptación | Estado | Evidencia / observado |
|---|---|---|---|
| BP-01 | `BP_PlayerController` hereda de `APMPlayerController`; referencia `IMC_Player` y las cinco IA; sensibilidad X/Y 1.0, inversión Y false, prioridad 0 y umbral 0.25 s; Event Graph vacío; compila y guarda. | NOT RUN | |
...
|---|---:|---|---|---|---|
| PLR-PIE-001 | 12.1 | Iniciar PIE en el Player Start. | GameMode crea Controller y Character; Controller posee el Character; BeginPlay añade `IMC_Player`; inicia solo la cámara 1P. No aparecen los warnings prohibidos de la sección D. | NOT RUN | |
...
| PLR-MOV-003 | 12.4 | Mover el ratón horizontal y verticalmente. | Yaw y pitch responden de forma continua, en el sentido base esperado y sin movimiento con el ratón quieto. | NOT RUN | |
...
| ID | Guía | Caso y procedimiento | Criterio de aceptación | Estado | Evidencia / observado |
|---|---:|---|---|---|---|
| PLR-PAD-001 | 12.19 | Con mando físico identificado, probar movimiento/look, sprint, crouch y cámara. | Todas las acciones responden; los sticks alcanzan rango útil y vuelven al centro. Registrar modelo, conexión y FPS. | NOT RUN | |
| PLR-PAD-002 | 12.19 | Dejar sticks en reposo, observar input/Pawn/cámara y recorrer el rango completo. | Se registra drift real. Si causa movimiento, definir una sola capa de zona muerta, preferentemente radial en Enhanced Input, y repetir. | NOT RUN | |
| PLR-CAM-005 | 12.20 | Cambiar sensibilidad X/Y desde defaults y comparar con 1.0. | Una entrada equivalente produce el cambio relativo esperado en cada eje; los valores negativos no se aceptan. | NOT RUN | |
...
| PLR-REG-003 | 12.22 | Detener PIE, compilar Blueprints, Save All, cerrar y reabrir UE 5.8. | Persisten IA/IMC, clases, defaults, GameMode y mapa; los Blueprints compilan e inicia Initial Mode. Este caso no prueba la preferencia de perspectiva. | NOT RUN | |
| PLR-REG-005 | Checklist oficial | Cambiar a 3P, cerrar la sesión y volver a iniciar el juego con el mismo perfil. | Se restaura la última perspectiva elegida. La implementación actual declara `CurrentMode` Transient, por lo que esta deuda probablemente producirá `FAIL` hasta implementarse; no reinterpretar el reinicio a Initial Mode como `PASS`. | NOT RUN | |

### Ciclo de vida y regresión controlada

| ID | Fuente | Caso y procedimiento | Criterio de aceptación | Estado | Evidencia / observado |
|---|---|---|---|---|---|
| PLR-REG-001 | Handoff / sec. 12 | Detener PIE con sprint o gesto de crouch activo, usando instrumentación que permita verificar `EndPlay`. | Limpia sprint y gesto transitorio; retirar `IMC_Player` no deja órdenes. No aprobar solo porque el actor desapareció. | NOT RUN | |
| PLR-REG-002 | Handoff / sec. 12 | Provocar `OnUnPossess` durante sprint y, por separado, durante un gesto de crouch activo, sin lógica central en Level Blueprint. | El Pawn anterior no conserva sprint; el gesto de crouch se cancela y restaura su postura inicial. Si no es observable, `BLOCKED` y automatizar. | NOT RUN | |
| PLR-REG-004 | Handoff | Repetir spawn, movimiento, crouch y cámaras después de reiniciar PIE. | No hay input/Mapping Context duplicado, estado transitorio heredado ni regresión respecto de los casos aprobados. | NOT RUN | |
...
| EVC-03 | Content Drawer, tipos de las cinco IA y mappings completos de `IMC_Player`. | NOT RUN | |
...
| EVC-06 | Output Log al iniciar y durante PIE; búsqueda de los cuatro mensajes prohibidos. | NOT RUN | |
| EVC-07 | Videos/mediciones de movimiento, crouch, cámaras, espacios y ciclo de vida. | NOT RUN | |
| EVC-08 | Mando identificado, valores analógicos, drift y única capa de dead zone. | NOT RUN | |
...
|---|---|---|
| Cambio estable 1P/3P | `PLR-CAM-001..003`, `PLR-REG-004` | NOT RUN |
...
| Inputs configurables en assets/defaults | `SET-02..03`, `MAP-01..06`, `BP-01`, `PLR-CAM-005..006`, `PLR-PAD-001..002` | NOT RUN |
...
| Guardar la perspectiva elegida | `PLR-REG-005` | NOT RUN — deuda conocida |
...
En particular, `PLR-REG-005` representa un requisito oficial todavía no
implementado. Mientras falle, la configuración del Editor puede documentarse,
pero v0.1.0 no puede aceptarse como versión completa.
...
- decisión sobre Jump;
- persistencia del modo 1P/3P entre sesiones;
...
- reasignación de controles en runtime y Gameplay Tags.
